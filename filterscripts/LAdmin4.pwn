...
public OnPlayerSpawn(playerid)
{
	...
	if(PlayerInfo[playerid][Frozen] == 1) {
		TogglePlayerControllable(playerid,false); return SendClientMessage(playerid,red," Вам не поможет esc, вы наказаны и все еще заморожены");
	}

	if(PlayerInfo[playerid][Jailed] == 1) {
	    SetTimerEx("JailPlayer",3000,0,"d",playerid); return SendClientMessage(playerid,red," Вам не поможет esc, вы наказаны и все еще в тюрьме");
	}
	...
	if(ServerInfo[AdminOnlySkins] == 1) {
		if( (GetPlayerSkin(playerid) == ServerInfo[AdminSkin]) || (GetPlayerSkin(playerid) == ServerInfo[AdminSkin2]) ) {
			if(PlayerInfo[playerid][Level] >= 1)
				GameTextForPlayer(playerid,"~b~DOBRO POJALOBAT~n~ ~w~ADMIN",3000,1);
			else {
...
public OnPlayerText(playerid, text[])
{
	...
	if(text[0] == '#' && PlayerInfo[playerid][Level] >= 1) {
		...
		format(string,sizeof(string)," Административный чат темного отряда: %s: %s",string,text[1]); MessageToAdmins(green,string);
	    return 0;
	}
...
dcmd_lhelp(playerid,params[]) {
	...
	if(PlayerInfo[playerid][LoggedIn] && PlayerInfo[playerid][Level] >= 1) {
		SendClientMessage(playerid,blue,"--== [ Админ-хелп ] ==--");
		SendClientMessage(playerid,blue, "Команды для админов:  /acmd   |   Авторы: /acredits");
		SendClientMessage(playerid,blue, "Система регистрации: /register, /login, /changepass, /stats, /resetstats.  Также:  /time, /report");
		SendClientMessage(playerid,blue, "Большое спасибо Дарку за помощь в написании этого скрипта.");
...
dcmd_lcommands(playerid,params[]) {
	...
	if(PlayerInfo[playerid][Level] > 11) {
		SendClientMessage(playerid,green,"11 левел: FLIP, JAIL, REPAIR, LP, CARCOLOUR, LTUNE, SETMYTIME, TIME, GETID, LINKCAR, LNOS, LHY");
		SendClientMessage(playerid,green,"11 левел: ANNOUNCE");
	}
	if(PlayerInfo[playerid][Level] > 0) {
		SendClientMessage(playerid,green,"1 левел: FLIP, FIX, REPAIR, LP, CARCOLOUR, LTUNE, SETMYTIME, TIME, GETID, LINKCAR, LNOS, LHY");
		SendClientMessage(playerid,green,"1 левел: ANNOUNCE, ANNOUNCE2, (UN) LOCKCAR, DISARM, GOTO, ADMINAREA, LCAR, LBIKE, LTC, CM (giveme), LSLOWMO, JETPACK");
	}
	if(PlayerInfo[playerid][Level] > 1) {
		SendClientMessage(playerid,COLOR_GREEN,"2 левел: SLAP, LSPEC (OFF), LSPECVEHICLE, BURN, WARN, MUTE, UNMUTE, GIVEWEAPON, GETINFO, LASTON, WRITE, CLEARCHAT, ASAY");
	}
	if(PlayerInfo[playerid][Level] > 2) {
	    SendClientMessage(playerid,green,"3 левел: SETCASH, SETSKIN, SETCOLOUR, SETWANTED, LMENU, LTELE, LVEHICLE, SETINTERIOR, LWEAPONS, LTMENU, SPAWN");
		SendClientMessage(playerid,green,"3 левел: KICK, EXPLODE,  EJECT, JAIL, UNJAIL, FREEZE, UNFREEZE, MOVE, LWEAPS, LAMMO, BOTCHECK");
    }
	if(PlayerInfo[playerid][Level] > 3) {
	    SendClientMessage(playerid,COLOR_GREEN,"4 левел: DUEL, COUNTDOWN, GETHERE, TELEPLAYER, CAR, CARHEALTH, DESTROY, LTIME, LWEATHER, CAPS, FORCE");
	}
	if(PlayerInfo[playerid][Level] > 4) {
    	SendClientMessage(playerid,green,"5 левел: SETHEALTH, SETARMOUR, SETGRAVITY, SETNAME, SETWEATHER, SETTIME, SETWORLD, HEALALL, ARMOURALL");
	}
    if(PlayerInfo[playerid][Level] > 5) {
	    SendClientMessage(playerid,COLOR_GREEN,"6 левел: SETALLWORLD, SETALLWEATHER, SETALLWANTED, SETALLTIME, GIVEALLWEAPON, SETALLSCORE, SETALLCASH, GIVEALLCASH");
	}
	if(PlayerInfo[playerid][Level] > 6) {
    	SendClientMessage(playerid,green,"7 левел: BAN, SETPING, DISABLE, ENABLE, FORBID (NAME/WORD), SETTEMPLEVEL, CC, (UN) INVIS, UCONFIG");
	}
	if(PlayerInfo[playerid][Level] > 7) {
	    SendClientMessage(playerid,COLOR_GREEN,"8 левел: KILLALL, DISARMALL, KICKALL, EJECTALL, FREEZEALL, UNFREEZEALL, SPAWNALL, (UN) LOCKSERVER, GOD, GODCAR");
	}
	if(PlayerInfo[playerid][Level] > 8) {
    	SendClientMessage(playerid,green,"9 левел: SLAPALL, UBOUND, FAKEDEATH, FAKECHAT, FAKECMD, CRASH, DIE, IP, BANKRUPT, LWEAPS, LHELI");
	}
	if(PlayerInfo[playerid][Level] > 17) SendClientMessage(playerid,COLOR_GREEN,"17 левел: SETLEVEL, OBJECT, PICKUP, (UN)LOADFS, GMX, RELOADLADMIN");
...
dcmd_disable(playerid,params[]) {
	...
	} else return SendClientMessage(playerid,red,"ОШИБКА: Вы должны быть темным администратором чтобы получить доступ к этой команде");
}
...
dcmd_acredits(playerid,params[]) {
	#pragma unused params
	return SendClientMessage(playerid,green,"Система Администрации. Спасибо большое Дарку, за помощь в написании этой системы!");
}
...
dcmd_teleplayer(playerid,params[]) {
    if(PlayerInfo[playerid][Level] >= 4 || IsPlayerAdmin(playerid)) {
		...
		if(PlayerInfo[player1][Level] == ServerInfo[MaxAdminLevel] && PlayerInfo[playerid][Level] != ServerInfo[MaxAdminLevel]) return SendClientMessage(playerid,red," ОШИБКА: Вы должны быть темным администратором чтобы получить доступ к этой команде");
	 	if(IsPlayerConnected(player1) && player1 != INVALID_PLAYER_ID) {
 		 	if(IsPlayerConnected(player2) && player2 != INVALID_PLAYER_ID) {
...
dcmd_adminarea(playerid,params[]) {
	...
	} return SendClientMessage(playerid,red, " ОШИБКА: Вы должны быть темным администратором чтобы получить доступ к этой команде.");
}
...
dcmd_reporting(playerid,params[]) {
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    dcmd(aconfig,7,cmdtext);
    dcmd(lhelp,7,cmdtext);
    dcmd(reporting,7,cmdtext);
	...
	dcmd(kill,4,cmdtext);
	dcmd(time,4,cmdtext);
	dcmd(acredits,5,cmdtext);
	dcmd(lcmds,5,cmdtext);
	dcmd(lcommands,9,cmdtext);
 	dcmd(acredits,8,cmdtext);
...
public PingKick()
{
	...
	for(new i = 0; i < MAX_PLAYERS; i++)
	{
		if(IsPlayerConnected(i) && PlayerInfo[i][Level] == 0)
		{
			...
			if(ammo > 1 && weap == 38) {
				new string[128]; format(string,sizeof(string)," ИНФО: у игрока %s содержится миниган с %d патронами", PlayerName2(i), ammo);
...
stock CMDMessageToAdmins(playerid,command[])
{
	...
	new string[128]; GetPlayerName(playerid,string,sizeof(string));
	format(string,sizeof(string)," Администратор %s использовал команду %s",string,command);
