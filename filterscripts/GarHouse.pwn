...
//------------------------------------------------------------------------------
	if(dialogid == 504)
	{
		...
		if(response)
		{
			...
			if(GetOwnedHouses(playerid) >= MAX_HOUSES_OWNED) { ShowInfoBox(playerid, INFORMATION_HEADER, "У Вас уже есть %d дома. Продайте один из старых домов и купите этот дом.", MAX_HOUSES_OWNED); return 1; }
