...
{
	if(!IsPlayerAdmin(playerid)) return SendClientMessage(playerid,-1,"{FF0000}ERROR: Вы не авторизованы в системе чтобы использовать эту команду!");
    Dialog_Show(playerid, Gate, DIALOG_STYLE_LIST, "{05A300}Меню ворот", "{FFFFFF}Создать ворота\n{FFFFFF}Редактировать ворота\n{FFFFFF}Удалить ворота\n{FFFFFF}Удалить все ворота\n{FFFFFF}Перезагрузить все ворота\n{FFFFFF}Включить пароль на ворота\n{FFFFFF}Отключить пароль на ворота\n{FFFFFF}Изменить пароль ворот", "Выбрать", "Отмена");
	return 1;
}
YCMD:gateinfo(playerid, params[], help)
{
	...
	for( new i = 0; i != MAX_GATES; i++ )
	{
		if(IsPlayerInRangeOfPoint( playerid, 10.0, GateInfo[i][gPosx], GateInfo[i][gPosy], GateInfo[i][gPosz]))
		{
			new
				string[128];
			format(string,128, "{FFFFFF}ИД ворот: %d |Модель ворот: %d | Позиция X: %.0f |Позиция Y:%.0f |Позиция Z:%.0f| Текст ворот: %s",GateInfo[i][gID],GateInfo[i][gModel],GateInfo[i][gPosx],GateInfo[i][gPosy],GateInfo[i][gPosz],GateInfo[i][gText1]);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(newkeys & KEY_YES)
	{
		...
		{
  			if( IsPlayerInRangeOfPoint( playerid, 10.0, GateInfo[i][gPosx], GateInfo[i][gPosy], GateInfo[i][gPosz] ) )
            if(GateInfo[i][ghavepass] == 1) return Dialog_Show(playerid,gatepass,DIALOG_STYLE_INPUT ,"{05A300}Пароль ворот","{FFFFFF}Напишите пароль от ворот\n»{FFCC00}Подсказка {FFFFFF}:Стандартный пароль: {FFCC00}1234. ","Select","");
  			{
...
Dialog:gatepass(playerid, response, listitem, inputtext[])
{
	...
	if(!strval(inputtext)) return SendClientMessage( playerid, -1, "»{FFCC00}Ворота{FFFFFF} - Пустой диалог." );
	}
	return 1;
}
