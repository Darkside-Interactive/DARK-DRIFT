...
public OnFilterScriptInit()
{
	...
	objd[0] = CreateObject(14797, 667.75000, -75.39060, 1048.31995,   0.00000, 0.00000, 0.00000);
	objd[1] = CreateObject(980, 658.96381, -75.38860, 1047.92798,   0.00000, 0.00000, -90.00000);//закрываем выход из гаража
	//интерьер 2
	obj2 = CreateObject(14783, 666.17188, -26.42970, 1048.96106,   0.00000, 0.00000, 0.00000);
	objd[2] = CreateObject(14796, 661.27338, -26.52340, 1050.68799,   0.00000, 0.00000, 0.00000);
	objd[3] = CreateObject(14826, 668.60162, -29.09380, 1047.69495,   0.00000, 0.00000, 0.00000);
	objd[4] = CreateObject(980, 658.83789, -26.55720, 1047.94604,   0.00000, 0.00000, -90.00000);//закрываем выход из гаража
	//интерьер 1
	obj3 = CreateObject(14776, 669.78912, 40.68750, 1056.43005,   0.00000, 0.00000, -90.00000);
	objd[5] = CreateObject(980, 653.67853, 40.95480, 1051.25903,   0.00000, 0.00000, -90.00000);//закрываем выход из гаража
#if (FS11INS == 1)
	objd[6] = CreateObject(19817, 663.96960, 38.74000, 1048.99597,   0.00000, 0.00000, -90.00000);//только для сервера 0.3.7 и выше !!!
	objd[7] = CreateObject(19817, 674.86853, 29.50200, 1049.10852,   0.00000, 0.00000, 180.00000);//только для сервера 0.3.7 и выше !!!
#endif
...
public OnPlayerCommandText(playerid, cmdtext[])
{
	...
	if(strcmp(cmd, "/gcreate", true) == 0)
	{
		if(IsPlayerAdmin(playerid))
		{
			...
			PickupID[para3] = CreateDynamicPickup(1318, 1, garagecordxin[para3], garagecordyin[para3], garagecordzin[para3], 0, 0, -1, 25.0);//создаём пикап гаража
			MapIconID[para3] = CreateDynamicMapIcon(garagecordxin[para3], garagecordyin[para3], garagecordzin[para3], 55, -1, 0, 0, -1, 200.0);//создаём мап-иконку гаража
			format(string, sizeof(string), "Гараж: Свободен\nСтоимость: %d\n/gbuy - купить гараж\nID: %d", garagecost2[para3], para3);
			Ngarage[para3] = CreateDynamic3DTextLabel(string, 0xADFF2FFF, garagecordxin[para3], garagecordyin[para3], garagecordzin[para3]+0.70, 25, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0);//создаём 3D-текст гаража
			...
			if(garagecount[para2] == 1)//дальнейшее выполняем если гараж существует
			{
				if(IsPlayerInRangeOfPoint(playerid, 4.00, garagecordxin[para2], garagecordyin[para2], garagecordzin[para2]))
				{
					if(strcmp(garageplayname[para2], "*** INV_PL_ID", false) == 0)//если это свободный гараж , то:
					{
						...
						new aa333[64];//доработка для использования Русских ников
						format(aa333, sizeof(aa333), "%s", sendername);//доработка для использования Русских ников
						printf("[Garage] Игрок %s [%d] купил гараж (ID: %d).", aa333, playerid, para2);//доработка для использования Русских ников
//						printf("[Garage] Игрок %s [%d] купил гараж (ID: %d).", sendername, playerid, para2);
			...
			if(gID >= 0 && gID < GARAGE_MAX)
			{
				...
				if(strcmp(garageplayname[gID], RealName[playerid], false) == 0)//если это гараж игрока, то:
				{
					...
					if(GetPlayerState(playerid) == 2)//если игрок на месте водителя, то:
					{
						...
						LinkVehicleToInterior(carpl, 0);//подключить транспорт к ТП интерьеру
						SetVehicleVirtualWorld(carpl, 0);//установить транспорту виртуальный мир игрока
						for(new i = 0; i < MAX_PLAYERS; i++)
						{
							if(IsPlayerConnected(i))
							{
...
#endif
#if (FS22INS == 2 || FS22INS == 3)
						CallRemoteFunction("StopDrift", "ddddffff", playerid, 7, 0, 0, garageanglein[gID],
						garagecordxin[gID]+CorX, garagecordyin[gID]+CorY, garagecordzin[gID]+2.00);
#endif
#if (FS22INS == 4)
						CallRemoteFunction("StopDrift", "ddddffff", playerid, 2, 0, 0, garageanglein[gID],
						garagecordxin[gID]+CorX, garagecordyin[gID]+CorY, garagecordzin[gID]+2.00);
#endif
					}
					else//иначе:
...
#endif
#if (FS22INS == 2 || FS22INS == 3)
						CallRemoteFunction("StopDrift", "ddddffff", playerid, 7, 0, 0, garageanglein[playgID2[playerid]],
						garagecordxin[playgID2[playerid]]+CorX, garagecordyin[playgID2[playerid]]+CorY, garagecordzin[playgID2[playerid]]+2.00);
#endif
#if (FS22INS == 4)
						CallRemoteFunction("StopDrift", "ddddffff", playerid, 2, 0, 0, garageanglein[playgID2[playerid]],
						garagecordxin[playgID2[playerid]]+CorX, garagecordyin[playgID2[playerid]]+CorY, garagecordzin[playgID2[playerid]]+2.00);
#endif
				...
				if(playgID1[i] >= 0 && playgID1[i] < GARAGE_MAX)
				{
					...
					if(CarTest(carpl) == 0)
					{
...
#endif
						{
							...
							SetPlayerInterior(i, 0);
							SetPlayerVirtualWorld(i, 0);
							LinkVehicleToInterior(carpl, 0);//подключить транспорт к 0-му интерьеру
							SetVehicleVirtualWorld(carpl, 0);//установить транспорту 0-й виртуальный мир
...
public LoadGarageSystem()//загрузка системы гаражей
{
	...
	for(new i; i < GARAGE_MAX; i++)
	{
		...
		if(file == INI_OK)
		{
			...
			PickupID[i] = CreateDynamicPickup(1318, 1, garagecordxin[i], garagecordyin[i], garagecordzin[i], 0, 0, -1, 25.0);//создаём пикап гаража
			MapIconID[i] = CreateDynamicMapIcon(garagecordxin[i], garagecordyin[i], garagecordzin[i], 55, -1, 0, 0, -1, 200.0);//создаём мап-иконку гаража
			...
			Ngarage[i] = CreateDynamic3DTextLabel(string, 0xADFF2FFF, garagecordxin[i], garagecordyin[i], garagecordzin[i]+0.70, 25, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0);//создаём 3D-текст гаража
			if(garagecarmod[i] != 0)//если есть сохранённый транспорт, то:
			{
				garagecarid[i] = CreateVehicle(garagecarmod[i], garagecordx[i],garagecordy[i],garagecordz[i], garageangle[i], -1, -1, 90000);//создаём транспорт в гараже
				LinkVehicleToInterior(garagecarid[i], garageint[i]);//подключаем транспорт к интерьеру гаража
