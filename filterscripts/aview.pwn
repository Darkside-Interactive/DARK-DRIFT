	...
	switch(dialogid)
	{
		...
		case 19113:
		{
			if(response)
			{
				...
				if(speed < 0.1000 || speed > 4.10000) return ShowPlayerDialog(EditID, 19113, DIALOG_STYLE_INPUT, "Speed", "{ff0000}Error value!     {c4c4c4}0.1 вЂ” 4.1\nSpecify speed. Default: 4.1", "Next", "Back");
				a_Speed = speed + 0.001;				
			}
			ShowSettingDialog();
		}
...
{
	new string[260];
	format(string, 260,"{c4c4c4}Скорость анимации\t\t{bf8e13}| {ffce63}%.1f\n\
						{c4c4c4}Цикл\t\t{bf8e13}| %s\n\
						{c4c4c4}Зацикленная позиция\t{bf8e13}| %s\n\
						{c4c4c4}Заморозить\t\t{bf8e13}| %s\n\
						{c4c4c4}Время анимации\t\t{bf8e13}| {ffce63}%i\n\
						{c4c4c4}Последующий цикл\t{bf8e13}| %s\n",
						a_Speed,
