...
COMMAND:skate(playerid,params[]){
        if(!IsPlayerInAnyVehicle(playerid)){
            ...
            if(!InfoSkate[playerid][sActive]){
                        ...
                        SendClientMessage(playerid,-1,"{00B100}Вы взяли скейт | Жмите ПКМ что-бы ехать");
                }else{
                        InfoSkate[playerid][sActive] = false;
                        DestroyObject(InfoSkate[playerid][sSkate]);
                        RemovePlayerAttachedObject(playerid,INDEX_SKATE);
                        PlayerPlaySound(playerid,21000,0,0,0);
                        SendClientMessage(playerid,-1,"{B00000}Вы убрали скейт.");
                }
        }else SendClientMessage(playerid,-1,"{00B7FF}Нельзя брать скейт когда вы находитесь в транспорте");
        return true;
}
